...
async function generateOneSlide(
  ...
  // Aggregate structure hints from summaries
  ...
  const systemPrompt = `你是学术导读PPT内容撰写专家。你的任务是根据全局规划和结构化元信息，为当前页生成完整的PPT内容。

## 你的身份
- PPT 单页内容撰写者
- 基于全局 slide plan 和结构化指令生成内容
...
你必须根据该页的 contentStructure 来组织内容块，而不是简单罗列要点：
...
### visualPriority → 内容重点
- **text**：以文字描述为主，多用 point 和 text
- **data**：以 finding 块突出数据和指标，文字只做辅助解释
...
- agenda：4-6个point块
- title-bullets：4-6个point块（仅此布局允许纯列表）
- two-column：heading + point/finding 交替（至少2个heading做栏分隔）
- comparison：heading + finding 交替（至少2个heading标示对比项）
- timeline：point块按时间/步骤排列，可用heading标示阶段
- data-card：3-5个finding块为主，可加summary做概述
- summary：2-4个summary/point块

## Notes 要求
notes 不是复读页面内容！
- mainTalk：这页该怎么讲（2-3句话，最多100字）
- extraExplanation：页面之外但讲者应知道的补充（最多100字）
- transitionSentence：过渡到下一页的话（1句话，最多50字）

## 质量要求
- 标题最多15字
- 每个要点15-30字完整句子
- 每页至少4个、最多8个内容块
- 如果有 keyNumbers，必须在 finding 块中体现
- 如果 groupingLogic 指定了分组方式，必须用 heading 块体现`;
...
请生成这一页的完整内容。
重要：
1. 内容组织方式必须服从 contentStructure="${plan.contentStructure || "linear"}"
2. 如果 groupingLogic 有指定分组方式，必须用 heading 块体现分组
3. 如果有关键数据，必须用 finding 块高亮展示
4. 不要所有页都退化成 point 列表`;
      ...
      properties: {
        ...
        contentBlocks: {
          ...
          items: {
            ...
            properties: {
              type: {
                type: "string",
                enum: ["point", "subpoint", "finding", "summary", "text", "heading"],
              },
              text: {
                type: "string",
        ...
        notes: {
          ...
          properties: {
            ...
            extraExplanation: { type: "string", description: "Max 100 chars" },
            transitionSentence: { type: "string", description: "Max 50 chars" },
          },
